...
For two terms $x$ and $y$ of the same type $A$, we would like to be able to state the proposition that the two terms are equal. For now, we merely consider equality between terms of the same type. This gives us a type former that takes a type as input, along with two terms of that type. In Typer, equality between terms is represented by the $\kw{Eq}$ type former\footnote{A type former can be thought of as a constructor that constructs a Type.} which has the below type signature.

\begin{minted}[escapeinside=@@,mathescape=true]{haskell}
Eq : (A : Type) @$\Rrightarrow$@ (x y : A) @$\rightarrow$@ Type
\end{minted}

The $\kw{Eq}$ type former takes a type as input, along with two terms of that type that we wish to claim are equal. We could then construct the type that stipulates that the addition of natural numbers is commutative by using the $\kw{Eq}$ type former.

\begin{minted}[escapeinside=@@,mathescape=true]{haskell}
Nat+_comm : (m : Nat) @$\rightarrow$@ (n : Nat) @$\rightarrow$@ Eq (m + n) (n + m);
\end{minted}

$\Funapp{\kw{Eq}}{(m + n)}{(n + m)}$ is merely a type and can be seen as a statement of the proposition that $m + n \equiv n + m$. In order to prove that the proposition is actually true, we would need to construct a term of this type. Such a term may be seen as a proof object that witnesses the validity of the proposition. Interested readers may find the complete proof of the commutativity of addition of natural numbers in the Appendix (TODO: Link appendix!). The one and only constructor of the $\kw{Eq}$ type is the following:

\begin{minted}[escapeinside=@@,mathescape=true]{haskell}
Eq_refl : (x : ?A) @$\Rrightarrow$@ Eq x x;
\end{minted}

Given a term $x$ of an arbitrary type $A$, we can trivially construct the proof that $x$ is equal to itself. This is known as the reflexive proof of equality. Although we say that our equality type is a proposition, it is by no means a mere proposition (HoTT book\cite{HoTTbook} Chapter 3.3). This implies that it is not a given that two proofs of the same equality are provably equal. In other words, when we ask ourselves the question  ``Is it the case that a = b'', we do not only care if the answer is yes or no, if they are indeed equal, we can interest ourselves in what ways exactly are they equal. This is in contrast to other systems where the uniquess of identity types (UIP) applies to all types. Notably, this is true of systems that admit axiom K \cite{streicher1993investigations} that states all identity proofs are equal to the reflexivity proof, and by extension all proofs of the same equality are equal. We will later show that UIP is a property that is true for a certain class of types in Typer.

- TODO: Maybe introduce the possible syntaxes for the type former, e.g. Id, $a=b$, Eq etc. (or just show one here and defer the examples for another section).
...
\begin{minted}[escapeinside=@@,mathescape=true]{haskell}
Eq_cast : (x : ?A) @$\Rrightarrow$@ (y : ?A) @$\Rrightarrow$@ (p : Eq x y) @$\Rrightarrow$@ (f : ?A -> Type)
          @$\Rrightarrow$@ f x -> f y;
\end{minted}

Note that it is possible to derive $Eq\_cast$ from the J-rule.


The natural that we should now ask ourselves is whether the J-rule can be derived from $Eq\_cast$, in other words we would like to know if the two elimination principles are equivalent. It turns out that one way of doing it hinges upon whether we are able to inhabit the following type:
...
Our first contribution in this paper involves proposing an alternative formulation of the $\kw{Eq}$ type based on an $\kw{Interval}$ type. Inspired by homotopy theory, the unit interval offers us an alternative way of modelling the equality type. For instance, an equality between two terms $x$ and $y$ can be seen as a path between the two terms where each term is precisely at an endpoint of the unit interval. Using the traditional formulation of equality types, many proofs regarding involving equality proofs require the use of the J-rule or transport. But this is often tedious and results in less intuitive proofs. An equality type based on the interval allows us to derive such proofs in an alternative manner that is much cleaner and more elegant. Examples of proofs that are simplified includ the congruence property of equality and a proof the commutativity of equality. An equality type based on the interval also allows us to derive function extensionality as a theorem.

- (TODO: mention this in the actual section) Also, we can prove the contractibility of singletons (as mentioned in https://arxiv.org/pdf/1611.02108.pdf). This proof is especially useful if we want to derive the J-rule.
...
\section{Syntaxique sugar}
TODO: Add after things get figured out

\chapter{Quotient Effectiveness}
