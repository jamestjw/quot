...
%% SM: You need some text here.
%% SM: Try and stick to one terminology rather than sometimes using
%% "identity" and other times "equality".
...
functions involving the equality type are universe polymorphic in their
...
%% SM: Then it's not an "axiom".  Don't confused our Builtin data constructor
%% with an axiom (I know I also sometimes call those things axioms, but it's
%% not right).
...
%% SM: You might want to say more explicitly that Heq_eq and Heq_uneq
%% correspond to your f&g functions in your earlier proof.
...
%% SM: Here you use `!` for the negation instead of `I_not`.
\begin{minted}[escapeinside=||,mathescape=true]{haskell}
...
\begin{minted}[escapeinside=||,mathescape=true]{haskell}
Eq_cong : (x y : ?A) |$\Rightarrow$| (g : (?A |$\rightarrow$| ?)) |$\rightarrow$| (p : Eq x y)
...
above are given in \autoref{sec:interval-connections}. Once we have that, we
may easily define the J-rule by performing a cast from \fn{P a Eq\_refl} to
\fn{P b p}:
...
%% SM: as before, you need text here.
...
\begin{prooftree*}
   ...
   \infer2[\textsc{Quot-Formation}]{\oftype{\ctx}{\Funapp{\kw{Quotient}}{\id{A}}{\id{R}}}
                                             {\kw{Type}}}
\end{prooftree*}
...
%% SM: Define what you mean by "effective" (and put it in \emph).
...
\begin{prooftree*}
   ...
   \infer[no rule]1{\oftype{\ctx}{\id{h}}{(\oftype{\id{a}_1 \ \id{a}_2}{\id{A}})
                                          ...
                                          \rightarrow \Funapp{\id{Heq}}{(\Funapp{\id{f}}{\id{a}_1})}
                                                                       {(\Funapp{\id{f}}{\id{a}_2})}}
   \infer1[\textsc{Quot-Elim}]{\oftype{\ctx}{\Funapp{\id{Quotient\_elim}}{\id{A}}{\id{R}}{\id{f}}{\id{h}}{\id{q}}}
                                        {\Funapp{\id{P}}{\id{q}}}}
\end{prooftree*}
...
%% SM: Usually this would be shown on a single line with no hypothesis.
\begin{prooftree*}
...
%% SM: The generalization of η to eliminators like Quotient_elim is
%% sufficiently different from the usual η that you need to spell it out.
%% SM: IIRC there are even different notions of η for such eliminators.
...
function, this would imply that $\id{Quotient\_elim}$ would need to be called with
...
shall examine each side of the square. To simplify the presentation, we shall
consider \id{sq} without the \id{Eq\_eq} constructor and the \id{Eq\_uneq}
deconstructor by treating functions out of the interval themselves as equality
proofs. This reduces \id{sq} to the following expression:
