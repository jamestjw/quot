...
$\kw{elim}_{\bI}^{_.A}(M, N , P, \ileft)$ is definitionally equal to \id{M}, we are
...
propositional equality between $\kw{cong}_{\kw{elim}_{\bI}^{A}(M, N,
  P)}(\id{seg})$ and P.
...
\begin{minted}[escapeinside=||,mathescape=true]{agda}
Integer_+-comm : (x y : Integer) |$\rightarrow$| Integer_+ x y |$\equiv$| Integer_+ y x

Integer_+-assoc : (x y z : Integer) |$\rightarrow$| Integer_+ x (Integer_+ y z) |$\equiv$| Integer_+ (Integer_+ x y) z

Integer_*-comm : (x y : Integer) |$\rightarrow$| Integer_* x y |$\equiv$| Integer_* y x

Integer_*-assoc : (x y z : Integer)
                  |$\rightarrow$| Integer_* x (Integer_* y z) |$\equiv$| Integer_* (Integer_* x y) z
\end{minted}
...
\begin{minted}[escapeinside=||,mathescape=true]{agda}
Integer_*DistL+ : (x y z : Integer)
                  |$\rightarrow$| Integer_* (Integer_+ x y) z |$\equiv$| Integer_+ (Integer_* x z) (Integer_* y z)
\end{minted}
...
\begin{minted}[escapeinside=||,mathescape=true]{agda}
Integer_+Linv : (x : Integer) |$\rightarrow$| Integer_+ (Integer_- 0 x) x |$\equiv$| 0

Integer_+Lid : (x : Integer) |$\rightarrow$| Integer_+ 0 x |$\equiv$| x
\end{minted}

Of course, we should also be able to say that 1 is the multiplicative identity.

\begin{minted}[escapeinside=||,mathescape=true]{agda}
Integer_*Lid : (x : Integer) |$\rightarrow$| Integer_* 1 x |$\equiv$| x
\end{minted}
...
\begin{minted}[escapeinside=||,mathescape=true]{agda}
Integer_isIntegral : (x y : Integer) |$\rightarrow$| Integer_* x y |$\equiv$| 0 |$\rightarrow$| (x |$\equiv$| 0 |$\rightarrow$| Void)
                     |$\rightarrow$| Eq y 0
\end{minted}
...
\begin{minted}[escapeinside=||,mathescape=true]{agda}
Integer_*Lzero : (x : Integer) |$\rightarrow$| Integer_* 0 x |$\equiv$| 0
\end{minted}
...
\begin{minted}[escapeinside=||,mathescape=true]{agda}
Integer_+Rid : (x : Integer) |$\rightarrow$| Integer_+ x 0 |$\equiv$| x
Integer_+Rid x = Eq_trans (Integer_+-comm x 0) (Integer_+Lid x)

Integer_+Rinv : (x : Integer) |$\rightarrow$| Integer_+ x (Integer_- 0 x) |$\equiv$| 0
Integer_+Rinv x = Eq_trans (Integer_+-comm x (Integer_- 0 x))
                           (Integer_+Linv x)

Integer_*Rid : (x : Integer) |$\rightarrow$| Integer_* x 1 |$\equiv$| x
Integer_*Rid x = Eq_trans (Integer_*-comm x 1) (Integer_*Lid x)

Integer_*Rzero : (x : Integer) |$\rightarrow$| Integer_* x 0 |$\equiv$| 0
Integer_*Rzero x = Eq_trans (Integer_*-comm x 0) (Integer_*Lzero x)

Integer_*DistR+ : (x y z : Integer)
                  |$\rightarrow$| Integer_* x (Integer_+ y z) |$\equiv$| Integer_+ (Integer_* x y) (Integer_* x z)
...
\begin{minted}[escapeinside=||,mathescape=true]{agda}
Integer_NegateDistL* : (x y : Integer)
                       |$\rightarrow$| Integer_- 0 (Integer_* x y) |$\equiv$| Integer_* (Integer_- 0 x) y
...
- Our quotient types work, but we see that in some cases they are not so easy to
use. Implementing quotient types in a system that doesn't have very good support
for proof writing makes them difficult to use. In the case of Typer, we lack
tactics and the specialisation of types in case branches.
...
\section{Equality Between \id{x} and \kw{cast}$_x$}\label{app:cast-is-eq}

For some term \id{x} of type \fn{A (_ := i0)}, we want to prove a heterogeneous
equality between \id{x} and the result of casting it to \fn{A (_ := i1)}.
...
We prove this using the special property of \id{I\_transp} described in
\autoref{sec:i-transp}. Since the $\eta$-equivalence \fn{A (_ := i)) $\equiv$
  (Eq_uneq (p := Eq_eq (f := A)) (i := i))} does not hold definitionally, we
prove it propositionally and use it to complete our proof.
