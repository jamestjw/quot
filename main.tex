...
%% SM: Here you use "↝" but in the computation rules for recI you used "=".
%% I think "↝" is preferable (if possible) since we already have
%% enough other uses for "=" around here :-)
...
%% SM: Maybe this should be moved to the beginning of this
%% subsection, and more generally it is worth pointing out that
%% in most proof assistants, functional extensionality is not provided
%% directly but requires the use of an axiom and that this tends to
%% introduce problems.
...
\end{minted}
%% SM: Read it aloud for your readers.
...
%% SM: "By appealing" make it sound like we have to request it explicitly,
%% and could be misunderstood that you need to add some annotation somewhere.
...
%% SM: The three lines above don't explain enough to understand the 4 lines
%% below.  Worse, it does not describe the same operation and uses
%% different names (A and B i.s.o `f x` and `f y`).
%% SM: You might want to explain it in terms of something like Leibniz equality.
%% SM: 
\begin{minted}[escapeinside=||,mathescape=true]{haskell}
