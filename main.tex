...
%% SM: The spacing is wrong after "i.e." because TeX thinks it's a full stop.
However, in order to do anything meaningful, we would need a way
to express the notion of equality, i.e. a crucial ingredient in expressing
...
%% SM: Yes, you should choose one and stick to it.  But it's good
%% to mention somewhere like here that it has other names.
...
%% SM: Hmm... we're not going to pass the proof to "conjunction", since
%% "conjunction" takes two *types* as arguments.
...
%% SM: The reader probably doesn't know what is "judgemental equality"
%% nor "metatheory".
...
%% SM: Maybe rather than "proofs are not executed during runtime" I'd go
%% with "we don't want proofs to be executed during runtime" (i.e. it
%% takes extra efforts to make sure this is the case).
...
%% SM: Here, you need to "read the type aloud" for the readers.
%% Presume they're mostly familiar with say Python or Java.  It's an
%% opportunity to describe the notation (including ":") you use, as well
%% as explain that this is a dependent type and what that means.
...
%% SM: The next paragraph says "also" but is not really connected to the
%% previous one.
%% SM: "extensionally" => "extensively".
%% SM: The way you wrote this may give the impression that it's used
%%   only in GHC (and maybe CompCert).  Try to write it to convey the idea
%%   that these are just some of the many places where it's used.
%%   Equality is used all over the place when you have more refined types.
...
%% SM: I recommend the Oxford comma.
...
%% SM: This gets number "0.1" but 0 is usually shunned in section
%% numbers, so maybe avoid sections in the intro or remove the "*"
%% from `\chapter`?
\section{Equality}
...
%% SM: Like before, you need to "read aloud" for the readers.  A bit
%% faster than last time, but still.  And acknowledge that it's a
%% "function" that takes and returns types rather than "values".
...
%% SM: This is not a "paper".
%% SM: Maybe it's still worthwhile at this point to remind them that
%% "the type" can be read as "the proposition".
...
%% SM: It's OK to take liberties with the code and syntax to hide details
%% under the rug, but don't introduce additional new syntax.  IOW, use
%% a simplified/idealized Typer syntax, rather than Haskell/Agda syntax.
...
%% SM: I think most programmers would not like to be able to say this.
%% Instead, they'd probably take it for granted that the type checker must know
%% that those two types are obviously the same.  So you first have to explain
%% that the type checkers are usually so dumb that they will not consider
%% that a vector of one type can be passed to a function expecting the other
%% type.  So, even though the programmers don't want to state this
%% equality, they end up needing to do it to convince the damn type checker
%% to accept their "obviously correct" program.
...
%% SM: I think you know what you need to do it, here.
...
%% SM: You're switching here to a different (sub)section, I suggest you
%% mark that somehow.  Until the end of this section, you're talking about
%% the fact that there are various different ways to introduce this notion
%% of equality, and you're also talking about erasure.  I suggest you
%% take those two one at a time.  Currently it's all mushed up in a single
%% somewhat breathless paragraph.
...
%% SM: Give a simple example of a quotient set early on, such as the
%% naturals modulo 256, or something.
...
%% SM: Define "inductive type" (tho, actually, here you can simply
%% remove "inductive").
...
%% SM: I think heterogenous equality might deserve a tiny bit more
%% explanation (too much for this section, so you'll need to talk about
%% it earlier), to explain that of course, "in reality" the two types are
%% equal.  You can give an example such as equality between
%% `x : Vector a (n + m)` and `y : Vector b (m + n)`.
%% SM: The footnote is very appreciated.
...
pathovers\cite{licata2015cubical} (TODO:\ The paper's definition of Heq has the
proof alpha that both types are equal, this concerns me a little bit as our
...
\autoref{ch:quotient-effectiveness}, this is required in order for the quotient
to be effective.
...
from that, we get defitional equalities even for path constructors.
...
equivalences as is done with $Qnorm\_eq$.
