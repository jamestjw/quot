...
We opted to implement quotient types as axioms in Typer, however we preserve canonicity by adding appropriate reduction rules wherever necessary (TODO: this particular sentence doesn't belong in the intro, I'll move it away). We introduced a quotient type former, term former and eliminator. We also define an equality constructor that is a witness of the equality of equivalent terms under the quotient. As a quick overview, we describe the construction of integers by using the constructs introduced in this work.
...
Here, we introduce the Quotient type former that takes a base type and an equivalence relation as arguments. Next, we demonstrate how a negation function may be defined on the integers. Since we interpret the integers as the difference in pairs of natural numbers, it suffices for us to flip the two underlying natural numbers of an integer in order to obtain its negation.
...
Z_negate z =
  let
    ...
    compat : (a b : Pair Nat Nat)
             -> (r : Eq ((fst a) + (snd b)) ((fst b) + (snd a)))
             -> equalZ (pair (snd a) (fst a)) (pair (snd b) (fst b));
    compat a b r =
      (snd a) + (fst b)
        ==< +-comm (snd a) (fst b) >==
      (fst b) + (snd a)
...
TODO: mention how quotient elimination involves proving that the operation respects the quotient. This is the opportunity to mention what Quotient_eq is all about and that it is very oten necessary during quotient elimination.
