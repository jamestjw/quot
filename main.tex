...
systematic way by leveraging the fact that the output type is a \kw{Prop}. First, we
need to prove a lemma, \id{castEqToHeq}. Suppose that we have a type former
$\id{A} \colon \id{I} \Rrightarrow \id{Type}$, two terms \id{x} and \id{y} of
...
\id{Quotient\_elim} to some function \id{f} and an surjection of some term
