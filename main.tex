...
%% SM: First occurrence of "HoTT", not yet defined.
...
%% SM: The `?A` and `:=` notations need to be briefly explained.
\begin{minted}[escapeinside=@@,mathescape=true]{agda}
...
and return a different term of type A depending on which endpoint of the
...
theories to provide a computational interpretation of the univalence
axiom\cite{bezem2014model}. This allows the encoding of equivalences as
equalities. This is much like what is done in informal mathematics where objects
with equivalent structures are just taken to be the ``the same''.

%% SM: Second occurrence of HoTT, still not defined.
...
%% SM: Clarify that this is a terminology used in Miranda and not universal.
...
%% SM: "the above" can be understood to refer to Miranda's laws.
...
%% SM: Spacing around `:` looks odd here and at many other places.
...
%% SM: "build" makes it sound like it's a runtime operation, whereas
%% it's usually understand as "writing the code".
...
%% SM: "only apply it" => "apply it only"
...
%% SM: AFAIK it's not quite the same as Courtieu's because the equalities
%% you get are not definitional.
...
coproducts, as is suggested in\cite[Chap~6.10]{HoTTbook}. This is unsurprising
given that Courtieu's implementation may also be translated to the Calculus of
Inductive Constructions\cite{werner-cic}, a close relative of Typer. However, we
%% SM: IIRC you can make it work for non-idempotent normalization functions
%% (or even for normalization of type `A -> B`) if you additionally
%% require an inverse function (when the normalization is idempotent,
%% the inverse function is the identity).  Eg. Cohen 2013's approach.
note that such a construction only works when the normalisation function is
idempodent.
...
\subsection{Equality}
%% SM: Mention that we're talking about "lines" and "paths" in the sense
%% used in mathematical topology.  It may help the reader.
...
\emph{paths} in the literature. If an equality between two terms is seen as a
line, then we may construct an equality between two lines to produce a square.
This process may be iterated to produce cubes, hypercubes, etc, justifying the
name of the type theory. Cubical Agda introduced the interval type as a
primitive with two endpoints \fn{i0 : I} and \fn{i1 : I}. Unlike normal types,
%% SM: Pattern matching is one particular way to observe differences,
%% so clarify that it's not only pattern matching that's disallowed.
pattern matching on an interval variable is disallowed. The type of homogeneous
equality is represented by $\equiv$ with the following type:
...
%% SM: In my world "brackets" are square, so either say "curly brackets"
%% or "braces".
...
%% SM: The fact that both are "x" can make it confusing.  Maybe change
%% your code to `refl {x = a} = λ i → a` to avoid the problem.
...
%% SM: "negates" makes sense if you think of I as the boolean type, but
%% "negating an endpoint" doesn't make sense in itself.  So I'd suggest
%% you say it swaps endpoints and then mention that the symbol chosen
%% is inspired from the boolean negation.
...
%% SM: "We provide" suggest it comes somewhere further than the current
%% sentence/paragraph.  How'bout "Here is a sketch of the proof:".
...
%% SM: I don't think you've talked about "the substitution property of
%% equality" yet.
...
%% SM: I'd use "an SMT (satisfiability modulo theories) solver" because the
%% full name makes the sentence harder to read.
...
%% SM: FWIW, in 2035, this kind of definition of Pair leaves the students
%% really puzzled.  So maybe use a different name for the term constructor,
%% and maybe use the `data Pair a where Pair : a -> a -> Pair a` syntax?
...
%% TODO: Again, verify that the formulation came from Hoffman
...
%% SM: Clarify the concrete difference between QIT and quotients.
...
%% TODO: This reflection can be brought elsewhere too I think, I don't have to
%% rush with this passage
...
%% an equivalence relation by adding appropriate constructors where necessary. A
%% similar definition in Quotient Haskell\cite{hewer2023quotient} is simpler and
%% more straight to the point, as is illustrated in the following code block:
...
%% TODO: Move this reflection elsewhere, to the evaluation chapter probably It
%% is valuable to mention what tradeoffs we get by going for own version of
%% quotients, but this seems like a deeper reflection that is not trivially
%% understandable
...
%% SM: Explain what is a "setoid structure" or move after the
%% setoid-rewrite section.
...
%% SM: "only required" => "required only"
...
\section{Typer's Equality Type}\label{sec:typer-old-eq}

%% SM: "before presenting cubical equality"??  You presented it in
%% a previous section.
We present the existing version of Typer's equality type before presenting
cubical equality. This is done notably to familiarise readers with Typer's
equality in general and to highlight the advantages of cubical equality in
subsequent chapters. This knowledge is nevertheless not necessary to understand
...
%% SM: Use \emph for "transport" since this is not talking about
%% moving around in bikes, buses, etc...
transport operation (also known as {\id{cast}}) allows us precisely to do the
above. We define the primitive \id{Eq\_cast} such that it takes a proof that
...
%% SM: A word is missing after "that".
we are left with a function that \fn{Eq x x} to \fn{Eq y x}, thus we pass it a
proof of reflexivity that \id{x} is equal to itself to obtain what we require.
...
%% SM: "higher inductive type" is not yet defined.
%% SM: "the HoTT book": Maybe that book is famous for us, but this might
%% be the first time the reader hears about it.
We first present the typical definition of the \emph{interval type} defined as a
higher inductive type as can be found in various sources such as the HoTT
book\cite[Chap~6.3]{HoTTbook}. We subsequently present our alternative
formulation of the interval type and show that it is equivalent to the
traditional definition. Finally, we define \emph{cubical} equality, a different
take on the equality type, based on the aforementioned interval type.

\subsection{Interval type}

In this section, we base our definitions on the assumption that we have an
existing equality type as was described in \autoref{sec:intro-eq}, i.e.\ that we
have a type former \fn{\_$\equiv$\_ : A $\rightarrow$ A $\rightarrow$ Type}
along with its constructor and eliminator. This helps us to introduce the
interval type and its relationship to the equality type. We will ultimately
define a new equality type that will then form the backbone of the rest of this
work.
...
elimination principle of $\bI$ using the \fn{elim$_{\bI}^{\_.A}$ (M, N, P, i)}
%% SM: $\_.A$ is the motive but not the *type*: only A is the type.
%% SM: Maybe it would be better to just use "A" without the "_." at this
%% point and adjust the syntax later if/when you need dependent elimination.
%% Because at this stage some readers will be very confused, a bit as
%% if you had written "because we're only dealing with wet water".
syntax. $\_.A$ is the \emph{motive} of the elimination, i.e.\ the type we wish
to eliminate to. The underscore indicates that the output type does not depend
on the interval term \id{i} that we wish to eliminate. \id{M} and \id{N} are
what we would like the output of the elimination to be when \id{i} is equal to
$\ileft$ and $\iright$ respectively, this is analagous to what we would place in
...
\begin{prooftree*}
   ...
   \infer3{\oftype{\ctx, \oftype{i}{\bI}}{elim_{\bI}^{\_.A} (M, N, P, i)}{A}}
\end{prooftree*}
...
\begin{align*}
  & elim_{\bI}^{\_.A}(M, N, P, \ileft) \rightsquigarrow{} M \\
  & elim_{\bI}^{\_.A}(M, N, P, \iright) \rightsquigarrow{} N \\
  & \kw{cong}_{elim_{\bI}^{\_.A}(M, N, P)}(\id{seg}) \rightsquigarrow{} P
\end{align*}

\kw{cong} refers to the congruence property of equality, if we have a proof
\id{p} of \fn{x $\equiv$ y} and some function \id{f}, then $\kw{cong}_f(p)$
constructs a proof of \fn{f x $\equiv$ f y}. We can expect the first two rules
to hold definitionally. Definitional equality of two terms is determined by
expanding out definitions. So when we say that $elim_{\bI}^{_.A}(M, N , P,
\ileft)$ is definitionally equal to \id{M}, we are saying that the former
reduces to the latter when we check for equality. If we imagine that the first
two computation rules were defined via pattern-matching, it would make sense for
these equations to hold definitionally. The third rule is a more curious case,
by the congruence of equality, the application of the $\kw{Interval}$ eliminator
with the equality proof $\id{P}$ to $\id{seg}$ should yield an equality proof
between $\id{M}$ and $\id{N}$. The rule suggests that this should compute to
$\id{P}$ itself. From a computational point of view, equalities between equality
proofs are only an important consideration in a proof-relevant setting, e.g.\ in
the context of HoTT. In the world of Typer where we demand that equality proofs
be erasable and play no role during computation, such equalities are less
meaningful. However, they are not completely irrelevant as we occasionally need
...
propositional equality between $\kw{cong}_{elim_{\bI}^{\_.A}(M, N,
  P)}(\id{seg})$ and P.
...
%% SM: You should maybe also discuss if that means we don't need the equality
%% type (which we presumed as pre-existing) any more.
We provide a proof of the above in Appendix~\ref{app:proof-i->a-equiv}.

%% SM: Why is this called "Justification"?
\subsection{Justification}\label{sec:eq-justification}
...
it because the argument is erasable. With that, we can postulate the following
equivalence.
...
%% section presents a funny rule I_transp with very little explanation about
...
%% and feel completely puzzled by this definition, just like you
%% found Cubical Agda's transport puzzlingly complicated at first.

%% SM: Why the negation?

%% Note: Hmm good point, probably not for any particular reason since i0 and i1
%% are symmetrical for all intents and purposes

We present a special elimination principle of our interval type. It is designed
to be a primitive upon which we would like to redefine the cast operation shown
in \autoref{sec:typer-old-eq}. Consider some term \id{x}, an equality proof
\id{p}, and an expression \fn{Eq\_cast (p := p) (f := id) x} which we denote as
\id{y}. It makes sense to say that \id{x} and \id{y} are related, although they
are not (necessarily) of the same type, they should still in some sense be
`equal'. The elimination principle that we implement aims to kill two birds with
one stone, not only does it implement the transport operation, it will
eventually also allow us to prove a heterogeneous equality between \id{x} and
\id{y}. It is largely inspired by the (generalised) transport operation used in
Cubical Agda\cite{vezzosi2021cubical}. It has the following typing rule:
...
%% TODO: I think I don't actually want to describe their rule? It's not easy to
%% make sense of and it wouldn't help people understand what's happening here
%% anyway

%% Since our version of $\id{I\_transp}$ is slightly different from that of
%% Cubical Agda, we do not need the additional side condition that states that A
%% needs to be a constant function when $\id{r} = \id{i}_1$ in order for the
%% above expression to typecheck. This is primarily because the ultimate return
%% type of the function is dependent on $\id{r}$ itself.
...
Eq_funExt : (f : (? |$\rightarrow$| ?)) |$\Rightarrow$| (g : (? |$\rightarrow$| ?))
            |$\Rightarrow$| (p : (x : ?) |$\rightarrow$| f x |$\equiv$| g x) |$\rightarrow$| f |$\equiv$| g
...
\begin{minted}[escapeinside=||,mathescape=true]{agda}
Eq_cong : (x y : ?A) |$\Rightarrow$| (g : (?A |$\rightarrow$| ?)) |$\rightarrow$| (p : x |$\equiv$| y) |$\rightarrow$| f x |$\equiv$| f y
...
overhaul of the existing system. In the rest of this section, we describe the
syntax of the new constructs followed by their typing and computation rules.

\subsection{Syntax}

We extend Typer's syntax of terms with our new constructs:

\setlength{\grammarparsep}{20pt plus 1pt minus 1pt} % increase separation between rules
\setlength{\grammarindent}{4em} % increase separation between LHS/RHS

%% Make the angle brackets go away
...
<\id{T}> ::= \ldots
\alt{} \fn{Quotient T T}
\alt{} \fn{Quotient_in T}
\alt{} \fn{Quotient_eq T}
\alt{} \fn{Quotient_elim T T T}
\end{grammar}
...
into the quotient \fn{Quotient A R}, we use the syntax \fn{Quotient\_in a}. The
%% SM: It's not at all clear here how/where R is specified in the projection.
projection requires us to specify what the underlying relation of the quotient
should be. Given that we have some evidence \id{r} that two terms \id{a$_1$} and
\id{a$_2$} are related by some quotient, we can construct an equality proof that
says that their projections into a quotient based on the aforementioned relation
are equal using the \fn{Quotient\_eq r} syntax. Finally, the elimination of a
term \id{q} of a quotient type is done using the syntax \fn{Quotient\_elim f h
  q} where \id{f} is a function out of the base type and \id{h} is proof that
\id{f} is compatible with the underlying relation of the quotient.
...
relation, though to define a meaningful quotient type, the above requirement is
%% SM: The reader may not know what is a "mere proposition" yet.
often necessary. We also do not enforce that the relation be a mere proposition,
%% SM: Maybe it's not worth mentioning this here, since we'd have to explain
%% also what is the significance of being effective.
however as we will see in \autoref{ch:quotient-effectiveness}, this is required
for the quotient to be \emph{effective}. We say that a quotient is effective if
it is the case that when the projections of two terms into a quotient are equal,
then the two terms are necessarily also related by the underlying relation of
the said quotient.
...
\begin{prooftree*}
   ...
   \hypo{\oftype{\ctx}{\Funapp{\id{Quotient}}{\id{A}}{\id{R}}}
                      {\kw{Type}}}
   \infer2[\textsc{Quot-Intro-In}]{\oftype{\ctx}{\Funapp{\id{Quotient\_in}}{\id{a}}}
...
\oftype{\ctx}{\Funapp{\id{Quotient\_eq}}{\id{r}}}
             {\Funapp{\id{Quotient\_in}}{\id{a}_1} \equiv \Funapp{\id{Quotient\_in}}{\id{A}}{\id{R}}{\id{a}_2}}}
\end{prooftree*}\label{quot-intro-eq-rule}

%% SM: Better name the relation than refer to it via "aforementioned".
By definition, given that two terms $\id{a}_1$ and $\id{a}_2$ of type $\id{A}$
are related by some relation, then the projections of these two terms into a
...
\begin{prooftree*}
   ...
   \infer[no rule]2{\oftype{\ctx}{\id{f}}
                           {(\oftype{\id{a}}{\id{A}}) \rightarrow \Funapp{\id{P}}{(\Funapp{\id{Quotient\_in}}{\id{a}})}}}
...
indexed by the quotient type itself. For instance, we could define the type of
integers as the quotient of pairs of natural numbers, which we denote by \fn{Z =
  Quotient (Nat $\times$ Nat) R} for some appropriate \id{R}. We might want to
%% SM: Some readers will think here "oh a thing that returns a boolean"...
define a function that determines whether a certain integer is positive,
%% SM: ... and then be stumped by "Dec" which is unknown to them.
%% IOW, it's a good example, but you need to explain "Dec" first.
i.e.\ of type \fn{(z:Z) $\rightarrow$ Dec (isPos z)}. This is known as dependent
elimination as the term that we are trying to eliminate, \id{z}, is itself
%% SM: I don't understand the "To that end" here.  It seems to refer to
%% something else than what you just wrote.  It smells like this text has been
%% moved around and needs to be adjusted to its new context.
present in the return type. To that end, we require some function $\id{f}$ that
takes some $\id{a}$ of the base type $\id{A}$ to
...
necessarily of the \textbf{same} type. We note as well that our dependent
%% SM: This note should probably be moved elsewhere.
elimination of quotients is not limited to motives that live in the universe of
\type{Prop}, as opposed to when quotient types are added to a theory interpreted
in a setoid model as proposed by Li\cite{li2015quotient}.

\subsection{Reduction rules}
We present the computation rule for the elimination of quotients.

\begin{prooftree*}
   \infer0{\Funapp{\id{Quotient\_elim}}{\id{f}}{\id{h}}{(\Funapp{\id{Quotient\_in}}{\id{a}})}
...
It is also interesting to note that we are not able to derive a useful
$\eta$-rule for quotient types. One instance of it that we could possibly define
is the following:

\begin{align*}
  & \Funapp{\id{Quotient\_in}}{(\Funapp{\fn{Quotient\_elim}}{\id{f}}{\id{h}}{\id{q}})} \rightsquigarrow{} \id{q} \\
\end{align*}
%% SM: There's also "Q_elim Q_in h q ↝ q (if the R is the same in both)".


This requires the condition that \id{f} be some function of type \fn{A
  $\rightarrow$ A} such that $\forall \id{a} \ . \ \fn{R a (f a)}$ is valid. For instance,
such a condition would be fulfilled if \id{f} was a normalisation function on
\id{A}. However, the above requirement is not easily enforced, not to mention
that the $\eta$-rule itself is not necessarily that useful to have.
...
%% SM: Use the ?x notation to simplify the types.
\begin{minted}[escapeinside=||,mathescape=true]{agda}
...
Quotient_eq : (A : Type) |$\Rrightarrow$| (R A |$\rightarrow$| A |$\rightarrow$| Type)  |$\Rrightarrow$| (a a' : A) |$\Rrightarrow$| R a a'
              |$\rightarrow$| Quotient_in (R := R) a |$\equiv$| Quotient_in (R := R) a'
\end{minted}
...
\begin{minted}[escapeinside=@@,mathescape=true]{agda}
PointwiseEq : (A : ?) @$\Rightarrow$@ (B : ?) @$\Rightarrow$@ (A @$\rightarrow$@ B) @$\rightarrow$@ (A @$\rightarrow$@ B) @$\rightarrow$@ Type_ ?;
...
\paragraph{Encoding of Quotient Inductive Types}

%% SM: This discussion uses terms not yet explained, and is hard
%% to follow for anyone not familiar with the topic.
%% Better either move it elsewhere or expand it substantially to
...
%% NOTE: Not sure what to name this
%% SM: Put the "x (Eq_cast (p := p) (f := id) x)" on it own line,
%% since it's the main meaning of the proof (or even elide the implicit
%% arguments, pretending that they can be inferred).
\begin{minted}[escapeinside=@@,mathescape=true]{agda}
h : (p : ?A @$\equiv$@ ?B) @$\rightarrow$@ (x : ?A)
    @$\Rrightarrow$@ Heq (t := @$\lambda$@ i @$\Rrightarrow$@ Eq_uneq (p := p) (i := i)) x (Eq_cast (p := p) (f := id) x)
...
\emph{Set} or 0-type in HoTT\cite[Chap~3.1]{HoTTbook}. We shall see in a
...
%% SM: This chapter is conceptually part of the qualitative evaluation of
%% your work, so you may want to use the term "evaluate/evaluation" here.
...
How the $\fn{ListPerm}$ relation has been presented so far suggests
...
extensionality.
%% SM: Earlier you said that quotient types can be defined (rather than added)
%% in a type theory with dependent pairs, which seems contradictory with
%% the above statement that quotient types allow us to prove funext
%% which is "usually" not provable.  Clarify what's going on here.
...
opposed to QITs that allow the direct definition of path constructors on
inductive types.

HITs and QITs are also strictly more expressive than the combination of ordinary
inductive types with quotient types\cite{fiore2022quotients}. However, in
practice, we think that the additional expressivity is often unrequired for
general programming. The example provided by the aforementioned paper of a QIT
that we may not trivially define using quotient types in the form of unordered
infinitely branching trees is indeed not a data type that one would expect to
make a frequent appearance in daily programming tasks. The simpler
...
%% SM: I take issue with "indeed": there isn't an apriori expectation that
%% it is the case.
%% And, it's not at all clear that they're more readable.
...
%% SM: How many times have you measured it?  Do you report the average of
%% all runs?  Or the minimum?  Can you include maybe the variance?

%% TODO: Column sizes aren't equal between zpair and Rational? How do I fix this?
\begin{table}[H]                %SM: As always, get rid of this `table[H]`.
\centering
\begin{tabular}{ccccc}
\hline
...
%% SM: This "indeed" is wrong, since you said we expected/hoped for
%% the opposite.
...
%% SM: `emacs-lisp-mode` would highlight "a b" as being placed in a way that
%% tends to confuse the reader.  I think you *need* a newline before them.
...
%% TODO: Supplement this with more explanations to make it more easily
%% digestable.
...
\begin{align*}
  ...
  & g \braket{x, \braket{y, p}} := \lambda i . \ elim_{\bI}^{\_.A}(x, y, p, i)
\end{align*}
...
\begin{align*}
  ...
  & \rightsquigarrow f \ (\lambda i . \ elim_{\bI}^{\_.A}(x, y, p, i)) \\
  & \rightsquigarrow \braket{x, \braket{y, \kw{cong}_{\lambda i . \ elim_{\bI}^{\_.A}(x, y, p, i)}(seg)}} \\
  & \rightsquigarrow \braket{x, \braket{y, \kw{cong}_{elim_{\bI}^{\_.A}(x, y, p)}(seg)}} \\
  & \rightsquigarrow \braket{x, \braket{y, p}} \\
\end{align*}

The third reduction uses the $\eta$-equivalence of functions. As for the final
reduction, we assume that the functorial action of $elim_{\bI}^{\_.A}(x, y, p)$
...
\begin{minted}[escapeinside=@@,mathescape=true]{agda}
sq : Heq (t := @$\lambda$@ i @$\Rrightarrow$@ x @$\equiv$@ Eq_uneq (p := p) (i := i))
         (Eq_refl (_ := x)) p
...
i.e.\ a proof that \fn{Eq\_refl (\_ := x)} is equal to \id{p}, we shall examine
...
@$\rightsquigarrow$@ @$\lambda$@ j @$\Rrightarrow$@ x
@$\rightsquigarrow$@ Eq_refl (_ := x)
\end{minted}
