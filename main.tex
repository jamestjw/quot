...
As a sanity check, we prove that the resulting $\kw{Rational}$ type is indeed a field as it should be. For brevity, we only provide the full proof of the negation operation, i.e.\ the operation that constructs the additive inverse of a rational number, the construction of the other operations shall merely be sketched. Negation is the simplest operation that we can define on the rationals, it is achieved by simply negating the integer numerator. This is done via the below function.
