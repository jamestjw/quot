...
terms may be constructed. As a simple example, the \id{Boolean} type is defined as
...
returns a default element in the event that the list is empty.
...
As we have seen before, the elimination principle of \id{Boolean} is the
...
element to the variable \id{a} and the rest of the list to \id{as}, and simply
...
\subsection{Comparison with Other Proof Assistants}\label{subsec:typer-comparison}
...
%% TODO: Mention this somewhere else after explaining Eq types
% - Lean has UIP
...
cannot be distinguished. As opposed to the \id{Boolean} type that has the
\fn{\kw{if}...\kw{then}...\kw{else}} construct that allows us to discriminate
between different values, we are not allowed to directly observe the value of
an interval term. Instead, we are provided with special built-in functions that
manipulate interval terms without requiring us to observe the underlying values
directly. In this alternative formulation of equality proofs, equality is
viewed as a continuous map from the unit interval to some type A. This
continuity reinforces our intuition as to why functions out of the interval
should not be able to inspect their arguments and return arbitrarily different
results for different terms of the interval type.
...
paradigm in Typer, with the objective of further familiarising the reader with
...
\section{Propositions as Types in Typer}
...
In a logic that is coherent, one should not be able to prove false
...
represents the type of non-negative integers is \fn{\{ i :\@ Int | i >= 0 \}}.
...
Lean has two universe of types, \kw{Prop} and \kw{Type}. \kw{Type} is analogous
...
type theory known as canonicity, which states that every term in a closed
context may be reduced to a canonical form. A term is of canonical form if it
is built up using only the constructors of its type. Using \id{Nat} as an
example, a closed term \fn{n :\@ Nat} must be able to be reduced to a chain of
applications of \id{succ} to \id{zero}. Type casts using equality proofs that
are axioms such as \id{Quot.sound} result in \emph{stuck terms}. Another
...
\id{u} is not of type \id{A}, or worse, \id{u} may not even type-check.
Our implementation of quotients shares similarities with \id{Lean}; however, we
aim to circumvent the aforementioned problems.
...
The elimination of $\bI$ is similar to that of \id{Boolean} as described in
...
branches during \id{Boolean} elimination. \id{P} is a proof that \id{M} and
...
distinct constructors that take no arguments, such as the \id{Boolean} type.
...
indexed by the quotient type itself. Note that unlike \Lean{}'s implementation
(see \id{Quot.ind} in \autoref{sec:intro-lean-quotients}), the type family
$\id{P}$ does not have to be a proposition, giving us more flexibility. To
construct an example of this, we first introduce a new definition. A type is
said to be \emph{decidable} if we can decide whether the type is inhabited or
not. In the case where it is inhabited, we must have a witness of such an
inhabitant. In Typer, decidability can be represented by the following
inductive type by using the `propositions as types' variation of logical
negation, \id{Not}, as introduced in \autoref{subsec:log-neg}.
...
explore the run-time cost of using quotient types in Typer. Quotient types are
...
value of the \id{Boolean} type, i.e., if it is not \id{true} or \id{false},
