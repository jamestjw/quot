...
%% TODO: Fix this
%% SM: You could cite "System F with Type Equality Coercions", which
%% basically describes GHC's internal language.
...
mathematical proofs (TODO: it might be cool to show some examples of props as
types, show some simple examples of conjunctions, disjunctions and
implications.). However, in order to do anything meaningful, we would need a way
to express the notion equality, i.e. a crucial ingredient in expressing
mathematical truths. In other words we require a type (TODO: ok might need to
improve the phrasing here) that plays the role of equality in mathematical
proofs. For instance, we might want to state the theorem that the addition of
natural numbers is commutative, i.e. $\forall xy. x + y \equiv y + x$. To this
end, our system requires an equality type (TODO: mention in footnotes other
names of this type. Also, I should choose a name and stick to it in the paper)
that allows us to make statements such as the above. Such a type provides us
with a proof object that can be manipulated like any other. This implies that we
can manipulate it with other operations such as logical negation, conjunction,
disjunction etc. This is in stark contrast to judgemental equality which only
exists in the metatheory and cannot be manipulated directly in programs.
...
Another notable example of the usage of proofs in real world programs is the
CompCert project that worked on the formal verification of a C compiler
...
type that we wish to claim are equal. We could then construct the type that
stipulates that the addition of natural numbers is commutative by using the
$\kw{Eq}$ type former.

\begin{minted}[escapeinside=@@,mathescape=true]{haskell}
Nat+_comm : (m : Nat) @$\rightarrow$@ (n : Nat) @$\rightarrow$@ Eq (m + n) (n +
m);
\end{minted}

$\Funapp{\kw{Eq}}{(m + n)}{(n + m)}$ is merely a type and can be seen as a
statement of the proposition that $m + n \equiv n + m$. In order to prove that
the proposition is actually true, we would need to construct a term of this
type. Such a term may be seen as a proof object that witnesses the validity of
the proposition. Interested readers may find the complete proof of the
commutativity of addition of natural numbers in the Appendix (TODO: Link
appendix!).
...
%% TODO: I used Agda syntax to simplify the presentation of this, maybe
%% encode this in Typer later in the appendix?
\begin{minted}[]{agda}
data Vector (A : Type) : Nat → Type where Vnil : Vector A 0 Vcons : {n : Nat} →
...
Vlength-comm+ : (m n : Nat) @$\Rrightarrow$@ Vector ?A (m + n) → Vector ?A (n +
m) Vlength-comm+ = lambda A m n @$\Rrightarrow$@ Eq_cast (p := Nat+_comm m n) (f
:= Vector A)
\end{minted}
...
normalisation function (TODO: mention Nuo Li and Courtieu). For these quotients,
it is possible to elect a canonical representative for each equivalence class.
In the rest of the paper, we first describe the typing and reduction rules of
the introduced type, and then we discuss the library that was implemented to
faciliate the usage of quotient types in Typer. Finally, we illustrate how the
...
of pathovers (TODO: explain/cite from somewhere). We also implemented a proof of
Hedberg's theorem \cite{hedberg1998coherence} in Typer. The theorem states that
if a type has decidable equalities, then its equalities are necessarily trivial.
Next, we introduced quotient types to Typer. This work then explores the
usability and convenience of quotient types in programming tasks. To facilitate
the use of quotients, we developed a library of convenience functions, macros,
and also theorems regarding quotient types. We also investigate the runtime cost
of the utilisation of quotient types. This is the first work that has attempted
to develop a substantial number of proofs in Typer. We introduce several
constructs to facilitate the writing of proofs in general in Typer in the form
of macros and axioms.

\section{Notes and TODOs}
- We have a proof of Hedberg's theorem (TODO: implement this!) to facilitate
eliminations to `Sets'. After explaining why this is necessary, we can motivate
the addition of the set truncation constructor for Quotients. If we can prove
that $\mathbb{Z}$ is a set when defined inductively (with the aforementioned
theorem), then it would be odd if the same cannot be done if $\mathbb{Z}$ was
defined as a quotient of $\mathbb{N} \times \mathbb{N}$. Indeed, this entails
bigger consequences (as mentioned in
https://staff.math.su.se/anders.mortberg/papers/cubicalagda2.pdf section 2.4.1).
(TODO: refer to my notes for a proof that Unit != Unit/R without the
truncation).


- Mention some caveats that we have, i.e. $cong \ id \ p \equiv p$ does not hold
definitionally in Typer, unlike in Cubical Agda (also mentioned in `A model of
type theory in cubical sets - Marc Bezem, Thierry Coquand, Simon Huber'). Of
course, if we could add an eta rule to make this work if we really wanted to
(TODO: Though how to add this without it being extremely ugly?).


- Apparently if Eq\_uneq doesn't reduce directly to either endpoint of the path
for concrete canonical values of I, then we can't derive funext directly. Not
sure if this fun fact is worth mentioning. (TODO: Mention this after the
reduction rules of Eq\_uneq)

- Implementing quotients by interpreting the theory in a setoid models (as done
by Nuo Li) results in a depelim that only works for Props (as opposed to what we
have)
