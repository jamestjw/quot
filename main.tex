...
book\cite[Chap~6.3]{HoTTbook}. We subsequently we present our alternative
...
function. This is the case when $\id{r} = \id{i}_1$, the type of
...
When $\id{r} = \id{i}_0$, then the type of
$\Funapp{\id{I\_transp}}{\id{A}}{\id{a}}{(\earg{\id{r}}{\id{i}_0})}$ is
$\Funapp{\id{A}}{(\earg{\id{i}_1})}$, this gives us a function that transports
an expression of type $\Funapp{\id{A}}{(\earg{\id{i}_0})}$ to type
$\Funapp{\id{A}}{(\earg{\id{i}_1})}$. We shall make use of this property
subsequently to define the actual transport function. The other
property of the $\id{I\_transp}$ function may appear bizarre at first sight, but
in reality it allows us to relate the transported value of \id{a} with \id{a}
itself by allowing us to prove that
$\Funapp{\id{I\_transp}}{\id{A}}{\id{a}}{(\earg{\id{r}}{\id{i}_0})}$ is equal to
...
type $(\id{r} : \bI) \Rrightarrow
\Funapp{\id{A}}{(\earg{\Funapp{\id{I\_not}}{\id{r}}})}$ and by evaluating this
function at both endpoints of $\bI$, we see that we have an equivalent of an
equality proof between
...
\begin{prooftree*}
  ...
  \infer1{\Funapp{\id{I\_transp}}{\id{A}}{\ileft}{\id{a}} \rightsquigarrow \id{a}}
\end{prooftree*}
...
\begin{minted}[escapeinside=@@,mathescape=true]{haskell}
I_transp : (A : I @$\Rrightarrow$@ Type) @$\Rrightarrow$@ A (_ := i0) @$\rightarrow$@ (r : I) @$\Rrightarrow$@ A (_ := I_not r)
\end{minted}
...
\section{Equality type}\label{sec:identity}
...
%% SM: Here you use `lambda` once and `λ` other times.  Stick to one.
\begin{minted}[escapeinside=@@,mathescape=true]{haskell}
...
%% SM: You can't presume the reader knows what is the "normal" equality
%% type.  I suggest you present Typer's old `Eq` type at some earlier time
%% (maybe in the new chapter between Intro and this one), so you can
%% refer to it here.
...
%% SM: You might want to compare it to the old `Eq_comm` proof.
\begin{minted}[escapeinside=||,mathescape=true]{haskell}
...
This often makes an appearance in mathematical proofs, it will especially be
required to prove the effectiveness of quotients in
...
then have to be postulated as an axiom, therefore adversely affecting
computation as we would describe below. By postulating the existence of the
$\type{Interval}$ type along with an equality type that's based on it,
functional extensionality can be derived as a theorem as we illustrate below.
...
%% SM: You need to explain what this code snippet does, step by step.
%% SM: It might be worth showing in a bit more detail where the reduction
%% gets stuck with funext as an axiom, since the readers may be
...
\begin{minted}[escapeinside=||,mathescape=true]{haskell}
Eq_cong : (x y : ?A) |$\Rightarrow$| (g : (?A |$\rightarrow$| ?)) |$\rightarrow$| (p : Eq x y) |$\rightarrow$| Eq (f x) (f y)
...
above. We define the function \id{Eq\_cast} such that it takes a proof that
...
Eq_cast_refl : (A : Type) |$\Rrightarrow$| (x : A)
               |$\rightarrow$| Eq (Eq_cast (p := Eq_refl (x := x)) (f := |$\lambda$| _ |$\rightarrow$| A) x) x
Eq_cast_refl A x = Eq_eq (f := |$\lambda$| i |$\Rrightarrow$| I_transp (A := |$\lambda$| _ |$\Rrightarrow$| A) (r := i) x)
\end{minted}
...
be done with the \id{Eq} type. After all, if all we had were proofs of the form
\fn{x $\equiv$ x}, it would be difficult to imagine achieving anything with
them. The true magic of the equality type lies in its elimination principle.
...
in\cite{martin1975intuitionistic}. Suppose that we have a type family \id{C}
that is parametrised by two terms \id{x} and \id{y} of some type \id{A} as well
as a proof that \id{x} and \id{y} are equal, i.e.\ of type \fn{Eq x y}. If we
have a proof term \id{p} that witnesses the equality of two arbitrary terms
\id{a} and \id{b} of type \id{A}, and we wish to produce a term of type \fn{C a
  b p}, then the J-rule says that it suffices for us to provide a means of
producing for all $\oftype{\id{x}}{\id{A}}$ a term of type \fn{C x x (refl x)}.
...
This rule is essentially saying that to know what to do with an equality between
an arbitrary \id{x} and \id{y}, we merely need to know how to handle the case
where we are dealing with a reflexivity proof. Now, given the fact that the
...
equality proofs are equal to the reflexivity proof, and by extension all proofs
...
%% FIXME: I think this should be moved to a later part of your thesis,
%% because it delays discussion of quotient types.
%% Maybe the same applies to some of the previous examples, actually.

%% TODO: Do this soon, I want to create a chapter with more complicated
%% examples, and leave the easy examples such as cong and funext with the eq
%% type presentation. This way, I can present rules of the eq type and quotient
%% types together in the same chapter
...
context of equality types. If we say that a type has decidable equality,
this implies that for any two terms of this type, we can decide whether or not
they are equal. On top of that, if the type has collapsed equality types, then
...
%% TODO: change this to section when I move this
In this chapter, we introduce our implementation of quotient types in Typer. We
first describe the syntax of the new constructs followed by their typing and
computation rules.

\section{Syntax}

We extend the Typer's syntax of terms with our new constructs:
...
%% SM: First present the syntax of the new elements (explaining what
%% each thing is supposed to mean), then present their typing rules.
%% The same holds for the previous chapter, actually.
Now, we present the typing rules of the primitives introduced above.

%% SM: Can we remove the implicit arguments to make the presentation lighter?
\begin{prooftree*}
...
%% SM: Give an example of dependent elimination on a quotient.
