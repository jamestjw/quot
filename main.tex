...
identical things may be substituted for one another. Thus if two terms are
equal, then if a predicate is true of the first term, then it is necessarily
...
%% NOTE: When defining a type on top of an existing type, the base type might
%% have `redundancies' as far as our purposes are concerned. Quotient types are
%% a way of taking advantage of the type checker to ensure that these
%% redundancies are appropriately handled internally. I think that instead of
%% saying that we can control what is ''exposed`, I would rather say that what
%% we end up exposing is necessarily `lawful' wrt to the quotient.
