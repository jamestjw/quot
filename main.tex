...
Equality proofs are also used extensionally in the internal language of GHC as
...
of addition of natural numbers in the Appendix (TODO: Link appendix!).
...
data Vector (A : Type) : Nat → Type where
    Vnil : Vector A 0
    Vcons : {n : Nat} → → Vector A n → Vector A (suc n)
\end{minted}
...
- We have a proof of Hedberg's theorem (TODO: describe this in the section of
equality) to facilitate eliminations to `Sets'. After explaining why this is
necessary, we can motivate the addition of the set truncation constructor for
Quotients. If we can prove that $\mathbb{Z}$ is a set when defined inductively
(with the aforementioned theorem), then it would be odd if the same cannot be
done if $\mathbb{Z}$ was defined as a quotient of $\mathbb{N} \times
\mathbb{N}$. Indeed, this entails bigger consequences (as mentioned in
...
arguments and return type.} and is implemented axiomatically.
...
\autoref{chap:quotient-effectiveness}, this is required in order for the quotient to be effective. (TODO: maybe link to more specific section instead of the chapter in general)
...
\chapter{Quotient Effectiveness}\label{chap:quotient-effectiveness}
