...
hold definitionally. The third rule is a more curious case, by the congruence of
equality, the application of the $\kw{Interval}$ eliminator with the equality
proof $\id{P}$ to $\id{seg}$ should yield an equality proof between $\id{M}$ and
...
play no role during computation, such equalities are less meaningful. However,
they are not completely irrelevant as we occasionally need to construct such
proofs for coherence purposes. In most cases, the third equation does not hold
definitionally as the \kw{cong} construct is defined independently and it would
be highly unusual to separately define how it should compute on new constructs
such as the interval type. In cases where we are unable to make the third rule hold
definitionally, we should then postulate a propositional equality between
$\kw{cong}_{elim_{\bI}^{\_.A}(M, N, P)}(\id{seg})$ and P.
...
that \fn{g} is a quasi-inverse of \fn{f} as described in \cite[Chap.
  2]{HoTTbook}. To this end, we define \id{f} and \id{g} as such:

%% TODO: I glossed over this detail, but the $elim_{\bI}$ actually carries out a
...
\begin{align*}
  ...
  & g \braket{x, \braket{y, p}} := \lambda \id{i} . \ elim_{\bI}^{\_.A}(\id{x}, \id{y}, \id{p}, \id{i})
\end{align*}
...
\begin{align*}
  ...
  & \id{$\alpha$-helper} (h , i) = elim_{\bI}(i . \ \eqtype{(g (f \ h)) \ i}{h \ i}, \ \kw{cong}_{\lambda i . \refl{h(i)}}(\id{seg}), \ i) \\
...
dependent pairs. And with that, the proof is complete.\end{proof}
