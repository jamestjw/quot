...
%% SM: I'd use `\emph` rather than quotes.
...
%% SM: Another example where equality is needed and that's usually easy
%% to grasp is the matrix product to ensure the matrices are compatible.
...
%% NOTE: I think I don't need to explain this. Delete when sure.
%%
...
%% SM: As mentioned in the abstract: add structure.
%% Usually I recommend the use of a few (typically 3-5) bullet points here.
...
%% TODO: Add proof of $\kw{cong}_2$ and $\Sigma$ path characterisation in the
%% appendix or something (sketch is in Interval.lagda.md)

%% Note: 23 Jan - Why did I bother to mention $\kw{cong}_2$ here? I can't
%% remember. Before I forget, $\Sigma$ path characterisation would explain why
%% $\beta$ is proven the way it is below.
...
\begin{minted}[escapeinside=@@,mathescape=true]{haskell}
Heq : (f : I @$\Rrightarrow$@ Type)
      @$\Rrightarrow$@ f (_ := i0) @$\rightarrow$@ f (_ := i1) @$\rightarrow$@ Type
\end{minted}
...
%% FIXME: Should this come before the section on funext? maybe, maybe not
...
reflexivity proof at $\fn{a}$. We provide more details of this in the appendix
...
quotient is said to be effective if given a proof that the injection of $\id{a}$
...
not described here\footnote{Interested readers may find the proof at
TODO:\ link}. However, once this has been proved, its counterpart is a trivial
corollary by virtue of the commutativity of $\zpairplus$.
...
$\kw{Quotient\_rec2}$ is a library function that allows us to simultaneously
...
produces its additive inverse. We define $\kw{Rational\_0}$ to be the additive
...
An empty multiset is simply the injection of the $\fn{nil}$ List into the
Quotient.
...
$\fn{isMember a $l_2$}$ is also valid, we omit this proof (TODO:\ Consider
putting this in the appendix, it's long). Next, we just need to define
$\fn{isMemberDec}$, whose details we shall once again omit. Finally, we may
proceed to define $\fn{Multiset\_mem}$.
...
TODO:\ Figure out what could be informative to say here. If this doesn't work
out, then ditch this section and rename the chapter or something
...
%% TODO: Wait this means that every single time I used the word `injection' into
%% the quotient, I really should have said `surjection' instead.
...
that act as projections out of the pair. Then, the injection into the quotient,
...
some function \id{f} and an injection of some term \id{x} into the quotient
...
%% TODO: Set the right tone for this chapter so that it can appropriately be put
%% after the introduction to give readers some background and motivation.
%% Or alternatively, I could split it up, show some of it as a
%% background/motivation and the half at the very end to compare approaches
...
%% TODO: 9 Feb, when rereading this, I can't seem to recall what I meant by
%% definitional equalities for path constructors
Elimination is done in cleaner manner when done using pattern matching. Aside
from that, we get definitional equalities even for path constructors.

TODO:\ Wanted to show an example of HIT-style elimination of a quotient, but I didn't find anything worth showing for now.

\section{Quotient Inductive Types}
...
definition of arbitrary equality constructors between terms of an inductive type
...
What is important to note here that since we are adding equality constructors,
...
of the properties of the equality type. Although it seems to be the case that
QITs are more convenient to work with in practice, we have not found any
evidence that they are more expressive than regular quotient types (TODO:\ I
better find a way to back up this claim). We opted to introduce plain quotient
types to Typer instead of QITs primarily because of its simpler implementation.
Aside from that, we get the added benefit of preserving the modularity of the
current implementation of inductive types.
...
transitive. In the theory, equality between two terms that are injections into
...
%% TODO: cite something, maybe A New Look at Generalized Rewriting in Type
%% Theory, though this seems more relevant
%% https://coq.inria.fr/doc/v8.12/refman/addendum/generalized-rewriting.html#first-class-setoids-and-morphisms
Plain rewriting allows rewriting with \emph{equal} terms, generalised rewriting
on the other hand enables rewriting using terms that are \emph{related} by some
binary relation (TODO:\ wait does this generalise to n-ary relations?), often
...
is compatible with the relation. (TODO:\ Maybe try to construct some example of
this in Coq) However, every function definition that interacts with the setoid
needs to be accompanied with a proof that setoid equivalence is respected. This
is in contrast to the ideal situation where such coherence proofs are
\emph{only} required in the function that eliminates quotients, subsequent
interaction and composition of this function should be free of the burden of
such proof obligations. This has historically been the go-to way of modelling
quotient types in Coq, however such an approach is known to be rather laborious.
This phenomenon is often designated as \emph{setoid hell} by the community.
...
An alternative way of defining a quotient on type $\kw{A}$ under a relation
$\kw{R}$ is to construct a function $\oftype{\id{nf}}{A \rightarrow A}$ such
that $\Funapp{\id{nf}}{\id{a}}$ is the canonical representative of the
equivalence class that $\id{a}$ belongs to. This approach is comparable
to\cite{courtieu-normalizedtypes,cohen2013pragmatic}. In other words, for
$\oftype{x, y}{A}$,
$\Funapp{\kw{Eq}}{(\Funapp{\kw{Quotient\_in}}{\id{x}})}{(\Funapp{\kw{Quotient\_in}}{\id{y}})}
\iff \Funapp{\kw{Eq}}{(\Funapp{\id{nf}}{\id{x}})}{(\Funapp{\id{nf}}{\id{y}})}$.
To represent this in our system, we can simply treat this as a quotient under
the relation $\Funapp{\id{R}}{\id{x}}{\id{y}} =
...
The proof itself is not very interesting, hence we do not describe it here.
(TODO:\ Link to proof). We note however that the reverse direction of this
equivalence gives us a means of eliminating $\kw{Qnorm}$, as was the case for
$\kw{Quotient}$.
...
by appealing to functional extensionality.

\begin{minted}[escapeinside=@@,mathescape=true]{agda}
e'e=id : (A : Type) @$\Rrightarrow$@ (B : Type) @$\Rrightarrow$@ (R : A @$\rightarrow$@ A @$\rightarrow$@ Type)
         @$\Rrightarrow$@ (f : Quotient A R @$\rightarrow$@ B)
         @$\rightarrow$@ Eq (e' (R := R) (e f)) f
e'e=id = @$\lambda$@ A B R @$\Rrightarrow$@ @$\lambda$@ f @$\rightarrow$@
  let
    ...
    p x f =
      Quotient_elim (R := R)
                    ...
                    (p := @$\lambda$@ x y r @$\rightarrow$@
                      Heq_cong (P := @$\lambda$@ x @$\rightarrow$@ Eq (f x) (f x))
                               (@$\lambda$@ x @$\rightarrow$@ Eq_refl (x := f x))
                               (Quotient_eq (R := R) (a := x) (a' := y) r))
                    x
  in
...
definitionally, however it may be proven propositionally as we show below:
...
proof of \fn{e $\circ{}$ e' $\equiv$ id}, we shall use the above two properties
manually by packing them into a lemma \id{$\Sigma${}$\equiv${}Prop} which states
that to prove that two terms of type \fn{Sigma A B} are equal, if
\fn{$\Pi_{x:A}$ isProp (B x)} is true, then it suffices to show that the
first projection of the two dependent pairs are equal. The proof of this lemma
is as follows:

%% TODO: Fix this proof to use the updated version of Heq that has an implicit
%% proof of equality in its type
...
@$\Sigma$@@$\equiv$@Prop = @$\lambda$@ l1 l2 A B @$\Rrightarrow$@ @$\lambda$@ propB u v p @$\rightarrow$@
  let
    ...
    pathOver : Heq (t1 := B u.fst) (t2 := B v.fst) u.snd v.snd;
    pathOver = whiskerRight (A := @$\lambda$@ i @$\Rrightarrow$@ B (Eq_uneq (p := p) (i := i)))
                            u.snd (Eq_cast (p := p) (f := B) (u.snd))
                            v.snd h sndv_eq;
    r : Heq (sigma (B := B) u.fst u.snd) (sigma (B := B) v.fst v.snd);
    r = Heq_eq (t := @$\lambda$@ _ @$\Rrightarrow$@ Sigma A B)
               (f := @$\lambda$@ i @$\Rrightarrow$@
...
By using the aforementioned lemma, we then construct a short and elegant proof
of \fn{e $\circ{}$ e' $\equiv$ id}.
