...
%% SM: Rather than "related" (which refers to some ambient relation, which
%% the reader may fail to understand), better say "equivalent" or
%% even more explicitly something like "equivalent according to
%% that relation" and then put emphasis on  "equivalent" and "equal" since
%% the role of the quotient is to turn an equivalence into an equality.

%% SM: "functions out of" may not be obvious to the reader either.
%% better talk more explicitly about the domain (or even the "argument")
%% of the function.
...
vectors, it could reasonably expect both inputs vectors to have the \emph{same}
length. Hence, the zipping of a \fn{Vector A (m + n)} and a \fn{Vector A (n +
  m)} would not typecheck. Below is a typical definition of the addition
...
type \fn{Vector A (m + n)} to produce one of the desired type. If we had a data
...
terms are equal, then if a predicate is true of the first term, then it is
...
types enables elements of the same equivalence class to be treated
...
case becase only programs that treat these equivalences consistently may be
...
%% SM: `\cite` where you got this definition.
...
%% SM: Replace "unit interval" here with "interval type I", and/or explain
%% why you call it "unit interval".
...
%% SM: I think the footnote should explain that there is no actual recursion.
...
%% SM: Here `rec` takes 2 args, but further below it takes 5!
%% SM: You'll typically get 3 kinds of readers here:
%% - Those who already know this stuff and will skip.
%% - Those who don't and will skip because it's too technical.
%% - Those who don't know but assume they should be able to follow,
%%   so they'll try and learn from what you write and they'll find those
...
%% SM: Terms like "definitionally" and "propositionally" should be explained
%% briefly before using them.
...
%% SM: Actually, they are important enough to need K or Hedberg.
...
%% SM: Relative precedence of those infix operators is not necessarily
%% fresh in the readers' mind (or worse, might be different in their own
%% field), so maybe sure you use extra spacing around ≃ to clarify the intent.
%% You have not talked about Σ yet.
\begin{align*}
  & \bI \rightarrow A \simeq \Sigma_{\oftype{x}{A}} . \Sigma_{\oftype{y}{A}} . \eqtype{x}{y}
\end{align*}
%% SM: Maybe use an actual `theorem` LaTeX environement and an actual
%% `proof` LaTeX environment.  So the reader who doesn't want to read the
%% proof can skip it more easily.


%% SM: There's been no mention of any `f` or `g` yet.
...
%% SM: Refer more precisely to where you showed/proved those things.
%% SM: Clarify "constant on the unit interval".
...
%% SM: I don't see where the type system enforces it.  AFAICT it's
%% enforced by the fact that the eliminator (`rec`) can only choose between
%% two things that are themselves equal.
...
\end{minted}
%% SM: Read aloud your example, especially since you can't presume the
%% reader knows Agda.

%% SM: What are those "explicit side conditions"?  You should compare
%% more pragmatically HITs to your quotients.  I.e. the fact that they
%% kind of "hide the non-quotiented base type", or the impact this has
%% when defining a recursive type.
...
%% FIXME: Avoid "h" in `figure`s: if you really want it "here" then just
%% don't use a `figure`.
\begin{figure}[H]
  \centering
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
...
%% SM: What do you mean by that?  Don't we "get" (or rather: have) to do
%% the same with your quotients?
from that, we get to specify how the function acts on equation constructors.
%% SM: In this inherent to the difference between HITs and quotients,
%% or is it a question of polish that could be added to your quotients?
...
%% SM: A->A is one option.  Another is "A->B": the "canonical
%% representative" doesn't have to belong to the same type.
%% When using a normalization function of type A->A you'll usually
%% want it to be idempotent (otherwise you have to make extra efforts
%% to remember if you've already applied the normalization function),
%% whereas with A->B this is not needed (or even meaningful)  :-)
