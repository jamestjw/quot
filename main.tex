...
TODO:Motivate equality types, what are they, why do we need them and what can we do with them?

I probably have more points here than necessary, extra stuff should go into other chapters

The `propositions as types' paradigm states that proofs are analogous to programs. Writing programs in the form of code is equivalent to writing mathematical proofs (TODO: it might be cool to show some examples of props as types, show some simple examples of conjunctions, disjunctions and implications.). However, in order to do anything meaningful, we would need a way to express mathematical truths, in other words we require a type (TODO: ok might need to improve the phrasing here) that plays the role of equality in mathematical proofs. For instance, we might want to state the theorem that the addition of natural numbers is commutative, i.e. $\forall xy. x + y \equiv y + x$. To this end, our system requires an equality type (TODO: mention in footnotes other names of this type. Also, I should choose a name and stick to it in the paper) that allows us to make statements such as the above. Such a type provides us with a proof object that can be manipulated like any other. This implies that we can manipulate it with other operations such as logical negation, conjunction, disjunction etc. This is in stark contrast to judgemental equality which only exists in the metatheory and cannot be manipulated directly in programs.

- an Eq type gives us something that we can manipulate with `logical' operations such as negation (with $\kw{Not}$), which cannot be done with judgemental equality. Can be seen as a predicate parametrised by a type A and two terms of A. Used to express mathematical truths, i.e. plays the role of equality in mathematical theories. Example: allows us to state that addition is commutative, i.e. $\forall xy. x + y = y + x$.

- Mention that our Eq type is a proposition, but not a mere proposition. Contrast this with other systems where UIP is a given.

- For two terms a, b of the same type A, we would like to be able to state the proposition that the two terms are equal (for now limit the discussion to terms of the same type only). This gives us a type former that takes a type as input, along with two terms of that type. A term of this type/proposition can thus be seen as a `proof' that a and b are indeed equal (maybe this is the right time to mention that terms of this type may or may not be unique, depending on the system we are working in. In other words, when we ask ourselves the question. is a = b, we do not only care if the answer is yes or no, if they are indeed equal, we also care about in what ways they are equal).

- Maybe introduce the possible syntaxes for the type former, e.g. Id, $a=b$, Eq etc. (or just show one here and defer the examples for another section).

- The intro rule of Eq only allows the construction of reflexive equalities, at first sight this leads us to believe that not much can be done with the Eq type. After all, we surely can't do much with a proof that x = x!? Perfect segue into the following point.

- Mention the elimination principle and justify it (the justification should probably in the dedicated section for Eq types, NOTE: check the alternative yoneda justification to see if its worth mentioning p216 HoTT-book) (J rule was introduced in Per Martin-Lof. An intuitionistic theory of types: predicative part 1973, though I think it was given this particular name in the paper, TODO: check this!)

- Maybe it's worth mentioning why it's fine to provide `cast' as a means of eliminating Eq types instead of providing `J'. (TODO: want to be able to prove that J can be derived from cast, by leveraging the fact that $(x , refl) = (y , p)$, check notes for more details).

- later (maybe not in the intro) draw a comparison with extensional identity types that admit equality reflecction (Martin-Löf 1982)

- Give code examples of the Eq type and what we can do with it.

Note: At some point, I'd like to mention that Eq types are not defined as `native' datatypes because our inductive types are not indexed datatypes. To encode indexed datatypes using our inductive types, we would require use of Eq types itself, i.e. we have a chicken and egg problem here. (Detailed explanation of this likely doesn't belong in the intro, could bring it up in passing in the intro.)

- Briefly mention the Interval, and say why we want to base an equality type on that

- Typically, many proofs regarding identity types require the use of identity elimination, i.e. the J-rule. But this is often tedious and less intuitive. An equality based on the interval allows to derive such proofs in an alternative manner that is much cleaner and more elegant.

- Example of proofs that are `simplified': congruence, eq-comm. We also get new `theorems' such as funext. Also, we can prove the contractibility of singletons (as mentioned in https://arxiv.org/pdf/1611.02108.pdf). This proof is especially useful if we want to derive the J-rule.


TODO: Motivate quotients, why are they useful and why do we care about them? Examples of things that can be made easier with quotients


In keeping with the theme of equality, we shall discuss the implementation of quotient types in Typer. Quotient types allow us to redefine the equality of a type, usually on the basis of an equivalence relaton on the type. This is inspired by quotients in set theory, where if we are given a set A and an equivalence relation $\sim$ on A, then we can construct the quotient set of A by $\sim$, which we denote as $A / \sim$ (TODO: examples of this). Equivalent elements in A are considered equal in $A / \sim$. The subsets of elements that are considered equal in $A / \sim$ are precisely the subsets whose elements are all equivalent under the equivalence relation. We can say that for some $a \in A$, the subset of $A$ such that all elements are equivalent to $a$ is $\{a' \in A \ | \ a' \sim a \}$. Indeed, this subset is what one would call the equivalence class of $a$, this is usually denoted as $[a]$. When we take the quotient of a set, we are essentially partitioning the set into equivalence classes, implying that we can regard a quotient set as simply the set of equivalence classes induced by the equivalence relation.

For a more concrete example, we could define the set of integers $\mathbb{Z}$ as a quotient of pairs of natural numbers $\mathbb{N}$. The set of pairs of natural numbers can be denoted as $\mathbb{N} \times \mathbb{N} = \{(x,y) : x \in \mathbb{N}, y \in \mathbb{N}\}$. The intuition behind this definition of the integers is that a pair of natural numbers can be interpreted as an integer if we consider the `difference' between the first and second natural number of a pair $(x, y)$ as the integer that is represented by the pair. For instance, we want the pairs $(0, 2), (1, 3), (2, 4)$  to all represent the integer $-2$. In other words, we would like to define the relation such as  $(x_1, y_1) \ R_{\mathbb{Z}} \ (x_2, y_2) = x_1 - y_1 \equiv x_2 - y_2$. However, natural numbers only typically support truncated subtraction, hence the better way of defining the relation would be to rewrite the equation using addition. The appropriate equivalence relation is $R_{\mathbb{Z}} = \{((x_1, y_1), (x_2, y_2)) \in (\mathbb{N} \times \mathbb{N})^2 \ | \ x_1 + y_2 = x_2 + y_1\}$. We define the relation the usual way by saying that it is a subset of the Cartesian product of the underlying set of the relation with itself. With that, we can define $\mathbb{Z} = \mathbb{N} \times \mathbb{N} / R_{\mathbb{Z}}$. Pairs that are equivalent under the relation $R_{\mathbb{Z}}$ are considered equal in the resulting quotient (set) $\mathbb{N} \times \mathbb{N} / R_{\mathbb{Z}}$.

We opted to implement quotient types as axioms in Typer, however we preserve canonicity by adding appropriate reduction rules wherever necessary (TODO: this particular sentence doesn't belong in the intro, I'll move it away). We introduced a quotient type former, term former and eliminator. We also define an equality constructor that is a witness of the equality of the injection of equivalent terms into the quotient. As a quick overview, we describe the construction of integers by using the constructs introduced in this work.

\begin{samepage}
\begin{minted}{haskell}
equalZ : Pair Nat Nat -> Pair Nat Nat -> Type;
equalZ
  | ((x1, y1), (x2, y2)) => Eq (x1 + y2) (x2 + y1);

Z = Quotient (Pair Nat Nat) equalZ;
\end{minted}
\end{samepage}

Here, we introduce the Quotient type former that takes a base type and an equivalence relation as arguments. To form a term of this type, we use the term former $\kw{Quotient\_in}$ which acts as a injection into the Quotient type. We later prove that this term former is in fact surjective (in the set theoretical sense).

\begin{samepage}
\begin{minted}{haskell}
Integer_0 : Z;
Integer_0 = Quotient_in (pair zero zero);

Z = Quotient (Pair Nat Nat) equalZ;
\end{minted}
\end{samepage}

The second Quotient term former is a witness that given two equivalent terms $a$ and $a'$, their injections into the Quotient are equal. For instance, we could prove that that the injection of $(2, 0)$ and $(3, 1)$ into the quotient are equal as they both represent the integer 2.

\begin{samepage}
\begin{minted}{haskell}
Zeq_2 : Eq (Quotient_in (pair (succ (succ zero)) zero))
           (Quotient_in (pair (succ (succ (succ zero))) (succ zero)));
Zeq_2 = Quotient_eq (a := pair (succ (succ zero)) zero)
                    (a' := pair (succ (succ (succ zero))) (succ zero))
                    Eq_refl;
\end{minted}
\end{samepage}

Since $2 + 1 \equiv 0 + 3$ is an equality that holds definitionally based on the way we defined addition on natural numbers, we can trivially fulfil the proof obligation by using $\kw{Eq\_refl}$. We shall see that such proof obligations are not always that easily fulfilled.

Next, we demonstrate how a negation function may be defined on the integers. Since we interpret the integers as the difference in pairs of natural numbers, it suffices for us to flip the two underlying natural numbers of an integer in order to obtain its negation.
...
Z_negate z =
  let
    ...
    flip p = pair (snd p) (fst p);

    compat : (a : Pair Nat Nat) -> (b : Pair Nat Nat)
             -> (r : Eq (a.fst + b.snd) (b.fst + a.snd))
             -> equalZ (pair a.snd a.fst) (pair b.snd b.fst);
    compat a b r =
      a.snd + b.fst
        ==< Nat+_comm a.snd b.fst >==
      b.fst + a.snd
        ==< Eq_comm r >==
      a.fst + b.snd
        ==< Nat+_comm a.fst b.snd >==
      b.snd + a.fst @$\qed$@;
  in
    qcase (z : (Pair Nat Nat) / equalZ)
     | Quotient_in a => Quotient_in (flip a)
     | Quotient_eq a a' r => Quotient_eq (flip a) (flip b)
                                         (compat a b r);
\end{minted}

In this example, we see that the usage of $\kw{qcase}$, i.e. the eliminator of quotients. Two things are required when eliminating a quotient, a map from the base type of the quotient to the output type of the elimination, and a proof that the aforementioned map respects the quotient. In the $\kw{Quotient\_in}$ branch of the eliminator, we get to `look inside' the quotient and use the underlying value. The $\kw{Quotient\_eq}$ branch is basically what quotient types are all about, we need to provide a proof that the mapping provided in the previous branch is compatible with the quotient. For a given $a$ and $a'$ that are equivalent with respect to the underlying relation, we need to produce a propositional equality between the action of the mapping on $a$ and $a'$. The elimination principle of quotient types essentially requires the coherent treatment of both the term constructor and the equality constructor.

Quotient types often provide us with alternative ways of defining inductive types that are familiar to us. Some of these definitions could potentially be better than the original formulations depending on the use cases of the resulting data type. For example, as was noted in \cite{hewer2023quotient}, by defining the List data type as a quotient of binary trees where the joining operation of two trees is associative, the resulting List datatype has a constant time append operation. Another example is if we consider the simple definition of integers as a normal inductive datatype based on natural numbers as such:
...
We would have two canonical representations of the integer 0, i.e. $\Funapp{\kw{pos}}{0}$ and $\Funapp{\kw{neg}}{0}$. Users of this Integer datatype would then be burdened with the responsbility of ensuring that both representations of zero are treated in the same manner in their programs. With quotient types, we could identify $\Funapp{\kw{pos}}{0}$ and $\Funapp{\kw{neg}}{0}$ such that well typed programs that use the quotiented integer type will necessarily have to be treat both representations of zero equally, in other words, we can let the type system do the book keeping for us.

We also show that there exists a class of quotients that can be defined without the addition of a new type former, i.e. quotients that are characterised by a normalisation function (TODO: mention Nuo Li and Courtieu). For these quotients, it is possible to elect a canonical representative for each equivalence class. In the rest of the paper, we first describe the typing and reduction rules of the introduced type, and then we discuss the library that was implemented to faciliate the usage of quotient types in Typer. Finally, we illustrate how the quotient type may be used by providing examples in the form of code along with commentary.

- Other contributions related to quotient types: a proof of Hedberg's theorem (TODO: implement this!) to facilitate eliminations to `Sets'. After explaining why this is necessary, we can motivate the addition of the set truncation constructor for Quotients. If we can prove that $\mathbb{Z}$ is a set when defined inductively (with the aforementioned theorem), then it would be odd if the same cannot be done if $\mathbb{Z}$ was defined as a quotient of $\mathbb{N} \times \mathbb{N}$. Indeed, this entails bigger consequences (as mentioned in https://staff.math.su.se/anders.mortberg/papers/cubicalagda2.pdf section 2.4.1). (TODO: refer to my notes for a proof that Unit != Unit/R without the truncation).


- Mention some caveats that we have, i.e. $cong \ id \ p \equiv p$ does not hold definitionally in Typer, unlike in Cubical Agda (also mentioned in `A model of type theory in cubical sets - Marc Bezem, Thierry Coquand, Simon Huber'). Of course, if we could add an eta rule to make this work if we really wanted to (TODO: Though how to add this without it being extremely ugly?).


Other notes (this won't go into the intro):
- Apparently if Eq\_uneq doesn't reduce directly to either endpoint of the path for concrete canonical values of I, then we can't derive funext directly. Not sure if this fun fact is worth mentioning.

- Implementing quotients by interpreting the theory in a setoid models (as done by Nuo Li) results in a depelim that only works for Props (as opposed to what we have)
...
\subsection{QITs and Quotient Haskell}
Points:
