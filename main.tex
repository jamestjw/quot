...
redefining the notion of equality for the base type. This is typically based on
...
%% SM: "Appropriate" here presumes the reader understands what we're
%% talking about.  Better replace it with something more concrete so the
%% readers can confirms that they understood correctly.
integers under an appropriate equivalence relation. To better accommodate the
introduction of quotient types to Typer, we revamped the built-in equality type
by drawing inspiration from cubical type theory and proof assistants that
implement it. As such, Typer's new equality type is based on functions that have
%% SM: "our new interval" suggests you already talked about it, which
%% you haven't, so tweak the previous sentence to introduce the term
%% "interval" there already.
our new interval primitive as an argument. Such an equality type is more
powerful and expressive, it notably lends itself well to the writing of proofs
related to quotient types. In this work, we also investigate the practicality of
the usage of quotient types in a language such as Typer, both in terms of
runtime efficiency and developer-friendliness. Quotient types do not exist in
most modern dependently typed languages, principally because their usage entails
burdensome proof obligations. To facilitate the usage of quotient types, we
provide a library that helps simplify the manipulation of quotient types. We
also leverage this new addition to the language to introduce new built-in types,
such as rational numbers, propositional truncation, etc. Finally, this work also
explores the development of proofs in Typer, to our knowledge this is the first
attempt to write a substantial amount of proofs using the language since the
language is primarily intended to be a general-purpose programming language. We
describe our experience and the challenges faced in the process. Additionally,
we introduce several constructs to streamline proof development in Typer.
...
%% SM: "proof of A and B" tends to be parsed as "proof of (A and B)",
%% which is not what you mean here.
prove that \fn{A} and \fn{B} are both true, we need a proof of \fn{A} and \fn{B}
...
%% SM: Lack of transition between the first and second sentence.
...
the right behaviour without incurring an additional runtime cost. In Typer, this
is mainly achieved through a certain feature that allows certain terms to be
marked as \emph{erasable}, a point that we shall come back to later. Consider a
function that accesses the i$^{th}$ element of an array with the below type:
...
%% SM: A bit rough transition between the previous paragraph and this one.
...
%% SM: "Equality proofs may also ..." suggests the previous context was talking
%% about equality proofs, which is wasn't.
...
equality between terms is represented by the $\kw{Eq}$ type former.

%% SM: I'd indent all those short code examples by a few spaces,
%% like `displaymath` and `quote` would do.
\begin{minted}[escapeinside=@@,mathescape=true]{agda}
...
%% SM: This shouldn't be a footnote since readers who skip it may not
...
%% SM: Rather then send readers to the appendix, I think we should refer
%% them to the next page where we present Eq_refl, (or just ask for their
%% patience).
...
%% SM: Don't assume the readers know the term "zip" for lists/vectors.
the latter type. Suppose we have a function that zips vectors, it could
reasonably expect both input vectors to have the \emph{same} length. Hence, the
zipping of a \fn{Vector A (m + n)} and a \fn{Vector A (n + m)} would not
type-check. Below is a typical definition of the addition operation on natural
numbers:
...
%% SM: The `?` in `(A $\colon$ ?)` may trip some readers, so better replace it
%% by `Type`.
\fn{(A $\colon$ ?) $\Rrightarrow$ (m n $\colon$ Nat) $\Rrightarrow$ Vector A (m
  + n) $\rightarrow$ Vector A (n + m)}. Here, the type of \id{A} is a
%% SM: "initialised to \id{Type}"?  Doesn't sound right.  Also, I'm not sure
%% we need to try and explain what's going on, at this point.
...
%% SM: You may want to mention `Nat+_comm` to help the readers make the
%% connection in case they forgot since last page that this is the
...
%% SM: Spacing on this line is a bit confusing: too big around `:` compared
%% to the space around `and`, encouraging the readers to misparse.
...
%% SM: AFAIK that notion existed before.  What he introduced there is
%% the John Major equality.
...
special property that both its endpoints should be indistinguishable. In this
...
%% SM: Spacing around / is uneven.
%% SM: "Equivalent elements in A" needs to say that it's equivalence according
%% to ~ rather than some other notion of equivalence.
which we denote as $A / \sim$. Equivalent elements in A are considered equal in
$A / \sim$. Conversely, the subsets of elements that are considered equal in $A
...
Indeed, this subset is what one would call the equivalence class of $a$, usually
denoted as $[a]$. When we take the quotient of a set, we are essentially
...
relation. For example, consider the integers modulo \id{5} where the underlying
%% SM: They "are" not inherently so, so better say that we choose them as
%% the canonical representatives.
relation is congruence modulo \id{5}. Here, 0, 1, 2, 3, and 4 are the canonical
representatives of their respective equivalence classes, or more specifically,
congruence classes.
...
%% SM: You could mention that those invariants/intentions may usually appear
%% as comments and that quotient types allow turning them into actual code
...
%% SM: Avoid passive "was implemented".
...
\begin{itemize}
  \item{First, we introduce a notion of heterogeneous equality based on the
    ...
    %% SM: Use present tense (same for the next few ıtems).
  \item{Next, we defined normal homogeneous equality as a special case of
    ...
    %% SM: "pathover" has not be explained yet.
  ...
  \item{As the first work to develop a substantial number of proofs in Typer,
    our experience lead us to introduce several constructs to facilitate the
...
type \id{A} are equal, a type family \fn{f $\colon$ A $\rightarrow$ Type} and
...
\begin{align*}
  & \Funapp{\id{Heq\_eq}}{(\earg{f}{\lambda i \Rrightarrow
      \Funapp{\id{Heq\_uneq}}{(\earg{p}{p})}{(\earg{i}{i})}})} \rightsquigarrow{} p
\end{align*}
...
\id{Not} is the propositions as type variation of logical negation. It is
defined based on the idea that if a proposition is \underline{not} true, then a
proof of the proposition would lead to a contradiction.
...
bottom), the type with no habitants.
...
\emph{decides} whether a certain integer is positive, i.e.\ of type \fn{(z
  $\colon$ Z) $\rightarrow$ Dec (isPos z)}. This is known as dependent
...
\begin{minted}[escapeinside=@@,mathescape=true]{agda}
Eq_@$\eta$@ : (A : I @$\Rrightarrow$@ Type) @$\Rrightarrow$@ (i : I) @$\rightarrow$@ A (_ := i) @$\equiv$@ Eq_uneq (p := Eq_eq (f := A)) (i := i)
...
We shall see in a subsequent section that this
...
\autoref{sec:prop-trunc} . We will see that the nature of the function $\id{f}$
has no bearing on our proof obligations, as they can be discharged in a very
systematic way by leveraging the fact that output type is a $\kw{Prop}$. First,
we need to prove a lemma, $\id{toPathOver}$. Suppose that we have a type former
$\id{A} \colon \id{I} \Rrightarrow \kw{Type}$, two terms $\id{x}$ and $\id{y}$
of types $\Funapp{\id{A}}{(\earg{\_}{\ileft})}$ and
...
% Text Node
\draw (361,102) node [anchor=north west][inner sep=0.75pt]  [rotate=-0.06] [align=left] {toPathOver};
% Text Node
  ...
  Quotient_elim
    ...
    (p := @$\lambda$@ a a' r @$\rightarrow$@
      let
        ...
        fa=fa' : Heq (f a) (f a')
        fa=fa' = toPathOver (A := @$\lambda$@ i @$\Rrightarrow$@ P (Eq_uneq (p := a=a') (i := i)))
                            (prop (Quotient_in a')
...
%% SM: "then two terms" => "then *the* two terms".
...
\begin{minted}[escapeinside=@@,mathescape=true]{agda}
propExt : (Erased ?A @$\rightarrow$@ Erased ?B) @$\rightarrow$@ (Erased ?B @$\rightarrow$@ Erased ?A) @$\rightarrow$@ Erased ?A @$\equiv$@ Erased ?B
\end{minted}
...
%% SM: Average over the 500 iterations or over what?
...
%% SM: The standard deviation is a large proportion of the average,
%% and is larger than the overhead.  That's a problem.  We don't necessarily
%% have to fix it (e.g. by re-running the benchmarks in such a way that
%% the SD is reduced), but we definitely need to discuss/explain it, because
%% it's definitely larger than what is usually considered normal.
%% Also the fact that SD is much smaller for one case (negation on ZxZ)
%% needs to be discussed/explained.
...
%% SM: Make this a \begin{figure}...\end{figure}...
\begin{mdframed}
\begin{minted}[escapeinside=@@,mathescape=true]{scheme}
(@$\lambda$@ (a) @$\rightarrow$@ (@$\lambda$@ (b) @$\rightarrow$@
...
%% SM: .. and refer to the figure here.  Clarify that these optimisations
%% are very simple/standard/common.
...
%% SM: The times are "too small": increase the number of repetitions to
%% bring it into the 1-2s seconds range.  Same as before, discuss/explain
%% the large value of the standard deviation.
\begin{table}[]
\centering
\begin{tabular}{@{}cccccc@{}}
\toprule
...
%% SM: Usually 100 repetitions of the test are considered sufficient
%% to get a good average and SD.  Better increase the time per repetition
%% instead.
...
%% SM: You should also say something about the benchmark results (e.g.
%% the fact that the overhead is large, why it's larger or smaller than
%% for the previous benchmark, ...).
...
%% SM: You didn't really implement "cubical equality": it's related,
...
Consider a type \id{A} and a type family \fn{B $\colon$ A $\rightarrow$ Type}.
...
\begin{minted}[escapeinside=@@,mathescape=true]{agda}
isProp@$\Pi$@ : (A : Type) @$\Rrightarrow$@ (B : A @$\rightarrow$@ Type)
          @$\Rrightarrow$@ (h : (x : A) @$\rightarrow$@ isProp (B x))
...
@$\Sigma$@@$\equiv$@Prop : (A : Type) @$\Rrightarrow$@ (B : (a : A) @$\rightarrow$@ Type)
         @$\Rrightarrow$@ ((x : A) @$\rightarrow$@ isProp (B x))
