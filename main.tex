...
type\footnote{In the literature paper, the terms `equality type', `identity
type' and `path' are used interchangeably. For our purposes, we shall employ the
term `equality type', though occassionally we shall use the term `path' to draw
an analogy with HoTT.} that allows us to make statements such as the above. Such
a type provides us with a proposition that can be manipulated like any other.
This implies that we can manipulate it with other operations such as logical
...
array. Dependent types are at play here as the type $\fn{i < length a}$ depends
on the terms $\fn{i}$ and $\fn{length a}$. When the function is called with all
...
accesses will never lead to a runtime error. Additionally, this could also allow
us to skip runtime bounds-checking as the index is necessarily valid as long as
